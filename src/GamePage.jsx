...
const GamePage = () => {
  ...
  const handleCanvasClick = (event) => {
    ...
    const randomGrassIndex = Math.floor(Math.random() * loadedGrassImages.length);
    const grassImg = new Image();
    grassImg.src = loadedGrassImages[randomGrassIndex];
    ...
    grassImg.onload = () => {
      if (selectedTool === 'grass') {
        ...
        const updatedGrasses = grasses.map((grass) => {
          if (Math.random() < 0.5 && grass.state < 5) {
            const newState = grass.state + 1;
            const newImg = loadedGrassImages.find(img => img.includes(`${Math.floor(newState / 5) + 1}-${newState % 5}`));
            let newSize = grass.size;

            if (newState === 5) {
              newSize *= 1.2; // Grow the size when reaching state 5
            }
